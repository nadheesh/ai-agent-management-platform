...
**Note:** For production deployments, replace the fake provider with a proper secrets backend (HashiCorp Vault, AWS Secrets Manager, Azure Key Vault, Google Secret Manager, etc.).
...
**Note:** Follow the OpenChoreo guide to create the required secrets.
...
**Note:** For production deployments, disable dev mode and configure persistent storage for OpenBao.
...
### Step 8: Install Build Extension
...
### Step 9: Install Evaluation Extension
