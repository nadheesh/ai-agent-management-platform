...
<details>
<summary>Don't have Gateway API CRDs, cert-manager, External Secrets Operator, or kgateway? Install them here</summary>
...
**Note:** For production deployments, replace the fake provider with a proper secrets backend (HashiCorp Vault, AWS Secrets Manager, etc.).
...
helm install openchoreo-control-plane \
  ...
  --create-namespace \
  --timeout 600s \
  --values https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.9.0/deployments/single-cluster/values-cp.yaml
...
helm install openchoreo-data-plane \
  ...
  --create-namespace \
  --timeout 600s \
  --values https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.9.0/deployments/single-cluster/values-dp.yaml
...
helm install openchoreo-build-plane \
  ...
  --namespace openchoreo-build-plane \
  --timeout 600s \
  --values https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.9.0/deployments/single-cluster/values-bp.yaml
...
helm install openchoreo-observability-plane \
  ...
  --timeout 900s \
  --values https://raw.githubusercontent.com/wso2/agent-manager/amp/v0.9.0/deployments/single-cluster/values-op.yaml
...
**For detailed plane registration steps (extracting CA certificates, creating secrets and creating plane CRs), refer to the [OpenChoreo Self-Hosted Kubernetes Guide](https://openchoreo.dev/docs/getting-started/try-it-out/on-self-hosted-kubernetes/).**
...
### Step 3: Install Secrets Extension (OpenBao)

The Secrets Extension provides secure secret management for the Data Plane using OpenBao (open-source fork of HashiCorp Vault).

```bash
# Set Secrets namespace
...
kubectl wait --for=condition=Available \
  deployment/amp-secrets-openbao -n ${SECRETS_NS} --timeout=300s
```

**Note:** The `openbao.server.dev.enabled=true` setting runs OpenBao in development mode with an in-memory backend. For production, configure persistent storage and proper authentication.

**Note:** This extension is required for runtime secret injection into deployed agents. The DataPlane and BuildPlane reference this secret store.
...
**Note:** The Evaluation Extension uses OAuth2 client credentials (via the AMP Publisher Client registered by the Thunder Extension) to authenticate when publishing scores to the Agent Manager. The `clientId`, `idpTokenUrl`, and `clientSecret` must match the `ampPublisherClient` settings used when installing the Thunder Extension. Pass the client secret via a values file to avoid exposing it in shell history:

```yaml
# publisher-auth-values.yaml
ampEvaluation:
  publisher:
    idpTokenUrl: "http://thunder.amp-thunder.svc.cluster.local:8090/oauth2/token"
    clientId: "amp-publisher-client"
    clientSecret: "<your-publisher-client-secret>"
```
...
helm install amp-evaluation-extension \
  ...
  --version 0.9.0 \
  --namespace ${BUILD_CI_NS} \
  --values publisher-auth-values.yaml \
  --timeout 1800s
```
